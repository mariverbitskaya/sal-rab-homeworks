...
function sendRequest(name, phone, address, goods, sum) {
  ...
  let data = {client:name + ' '+ phone,order:{address:'ул. '+ address.street + ', дом '+ address.house +', '+ address.entrance +' подъезд, '+   address.floor +' этаж, кв ' +  address.flat , sum}, goods:myarray};
 
    let countOfGoods = goods.length;
    //Object({ title: 'Хачапури', count: 5 }) ] })
    for (let i = 0; i < countOfGoods; i += 1) {
        let mytaple={title:goods[i].title, count:goods[i].count};
        data.goods.push(mytaple);
    }
    
  // data.client = {client:name + ' '+ phone};
